...
class GeolocationResult(BaseModel):
    ...
    status: str = Field(description="Status: success, error, timeout")
    analysis: str = Field(description="Geolocation analysis text output")
    confidence: Optional[str] = Field(default=None, description="Confidence level")
    location: Optional[dict] = Field(
        default_factory=dict, description="Location data (country, city, etc.)"
    )
    coordinates: Optional[dict] = Field(
        default=None, description="Lat/lon coordinates if estimated"
    )
    key_clues: Optional[list] = Field(
        default_factory=list, description="Key visual clues used"
    )
    error: Optional[str] = Field(default=None, description="Error message if failed")
